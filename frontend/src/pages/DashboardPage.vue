    ...
    <header class="bg-white shadow-sm">
      <div class="max-w-6xl mx-auto px-4 py-4 flex justify-between items-center">
        ...
        <div class="flex items-center gap-4">
          ...
          <button @click="handleLogout" class="text-sm text-red-600 hover:text-red-800 font-medium">Sair</button>
        </div>
      </div>
    </header>
    ...
    <main class="max-w-6xl mx-auto px-4 py-8 space-y-6">
      ...
      <div class="grid grid-cols-3 gap-4">
        <button
          ...
          :key="action.type"
          @click="showModal(action.type)"
          class="bg-white rounded-xl p-5 shadow text-center hover:shadow-md transition"
        >
          <div class="text-2xl mb-2" :class="action.color">{{ action.icon }}</div>
        ...
        <div v-else class="divide-y divide-gray-100">
          ...
          >
            <div class="flex items-center gap-3">
              <div class="w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold" :class="txIconClass(tx)">
                {{ txIcon(tx) }}
              </div>
              <div>
            ...
            <span class="text-sm font-semibold" :class="tx.type === 'deposit' ? 'text-green-600' : 'text-red-600'">
              {{ tx.type === 'deposit' ? '+' : '-' }}{{ walletStore.formatCurrency(tx.amount) }}
            </span>
          </div>
        </div>
      </div>
    ...
    <div v-if="modal.show" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
      <div class="bg-white rounded-2xl p-6 w-full max-w-md mx-4 shadow-xl">
        <h3 class="text-lg font-bold text-gray-900 mb-5">
          {{ actions.find(a => a.type === modal.type)?.label }}
        </h3>
        ...
        <form @submit.prevent="handleAction" class="space-y-4">
          ...
          <div v-if="modal.error" class="bg-red-50 text-red-600 text-sm px-4 py-3 rounded-lg">{{ modal.error }}</div>
          <div v-if="modal.success" class="bg-green-50 text-green-700 text-sm px-4 py-3 rounded-lg">{{ modal.success }}</div>
          ...
          <div class="flex gap-3 pt-2">
            ...
            <button type="submit" :disabled="walletStore.loading"
...
const actions = [
  { type: 'deposit',  label: 'Depositar',   icon: '+', color: 'text-green-500' },
  { type: 'withdraw', label: 'Sacar',        icon: '-', color: 'text-red-500'   },
  { type: 'transfer', label: 'Transferir',   icon: '→', color: 'text-blue-500'  },
]
...
function showModal(type) {
  Object.assign(modal, { show: true, type, amount: null, description: '', recipientId: '', error: '', success: '' })
}
...
async function handleAction() {
  ...
  modal.success = ''
  try {
    if (modal.type === 'deposit')       await walletStore.deposit(modal.amount, modal.description)
    else if (modal.type === 'withdraw') await walletStore.withdraw(modal.amount, modal.description)
    else                                await walletStore.transfer(modal.recipientId, modal.amount, modal.description)

    modal.success = { deposit: 'Depósito realizado!', withdraw: 'Saque realizado!', transfer: 'Transferência realizada!' }[modal.type]
    await walletStore.fetchHistory(10)
