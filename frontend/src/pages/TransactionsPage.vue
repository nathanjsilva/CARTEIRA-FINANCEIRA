        ...
        <div v-else class="divide-y divide-gray-100">
          ...
          >
            <div class="flex items-center gap-4">
              <div class="w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold" :class="txIconClass(tx)">
                {{ txIcon(tx) }}
              </div>
              <div>
                <p class="font-medium text-gray-800">{{ txLabel(tx) }}</p>
                <p class="text-xs text-gray-400 mt-0.5">{{ formatDate(tx.created_at) }}</p>
              </div>
            </div>
            <div class="text-right">
              <p class="font-semibold" :class="tx.type === 'deposit' ? 'text-green-600' : 'text-red-600'">
